...
/* ======================================================
   KONFIGURASI FORM (WAJIB DIATUR PER HALAMAN)
====================================================== */
const FORM_TYPE = "apd"; 
// ganti jadi "cuci_tangan" di halaman cuci tangan

/* ======================================================
   STORAGE KEY (100% TERPISAH)
====================================================== */
const STORAGE_KEY = `observasi_fields_${FORM_TYPE}`;

/* ======================================================
   DEFAULT FIELD PER FORM
====================================================== */
const defaultFieldsMap = {
  ...
  cuci_tangan: [
    { name: "air_mengalir", label: "Air Mengalir" },
    { name: "sabun", label: "Sabun" },
    { name: "gosok_tangan", label: "Gosok Tangan" },
    { name: "bilas", label: "Bilas" },
    { name: "keringkan", label: "Keringkan Tangan" }
  ]
};

/* ======================================================
   LOAD DATA (AMAN & TERPISAH)
====================================================== */
let fields =
  JSON.parse(localStorage.getItem(STORAGE_KEY)) ||
  structuredClone(defaultFieldsMap[FORM_TYPE]) ||
  [];
  ...
  fields.forEach((item, index) => {
    table.innerHTML += `
      <tr>
        ...
        <td data-label="Nama Field">
          <span class="field-code">${item.name}</span>
        </td>
        <td data-label="Label">${item.label}</td>
        <td data-label="Aksi">
          <div class="action-btn">
            <i class="fa-regular fa-pen-to-square edit"
               onclick="openEdit(${index})"></i>
            <i class="fa-regular fa-trash-can delete"
               onclick="openDelete(${index})"></i>
          </div>
        </td>
      </tr>
    `;
...
function enableMobileCardToggle() {
  ...
  const rows = document.querySelectorAll("#fieldTable tr");
  rows.forEach(row => {
