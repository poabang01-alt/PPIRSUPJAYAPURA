...
const STORAGE_KEY = "laporan_cuci_data";
...
function initCharts(){
  ...
  if(window.pieCuciChart instanceof Chart) window.pieCuciChart.destroy();
  ...
  /* ===============================
     PIE CUCI TANGAN
  =============================== */
  window.pieCuciChart = new Chart(
    document.getElementById("pieCuci"),
    {
      type:"pie",
      data:{
...
const tabelData = JSON.parse(localStorage.getItem(STORAGE_KEY)) || [
  ...
  {
    no:4,
    auditor:"Riko Saputra",
    pegawai:"Dian Lestari",
    tindakan:"Observasi",
    ruangan:"HCU",
    haircap:"Ya",
    faceshield:"Ya",
...
if(!localStorage.getItem(STORAGE_KEY)){
  localStorage.setItem(STORAGE_KEY, JSON.stringify(tabelData));
}
  ...
  filteredData.forEach(r=>{
    ...
    html+=`
      <tr>
        ...
        <td class="status-cuci">${r.jenisCuci || "-"}</td>
...
document.addEventListener("click",(e)=>{
  ...
  // 🔒 Jangan aktif kalau klik header
if(cell.closest("thead")) return;
  ...
  // ===============================
  // TOGGLE JENIS CUCI (INDEX 13)
  // ===============================
  if(col === 13){

    row.jenisCuci = row.jenisCuci === "Handwash"
      ? "Handrub"
      : "Handwash";

    cell.classList.add("cell-edited");
    setTimeout(()=> cell.classList.remove("cell-edited"),600);

    localStorage.setItem(STORAGE_KEY, JSON.stringify(tabelData));

    // re-apply filter kalau ada
    filteredData = [...tabelData];
...
filteredData.forEach(r=>{
  if(r.jenisCuci === "Handwash") handwash++;
  if(r.jenisCuci === "Handrub") handrub++;
});

if(window.pieCuciChart){
  window.pieCuciChart.data.datasets[0].data = [handwash, handrub];
  window.pieCuciChart.update();
}
}
...
function exportTableNow(){
  ...
  XLSX.writeFile(wb,"Laporan_Cuci_Tangan.xlsx");
}
